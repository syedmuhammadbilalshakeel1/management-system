...
const initialEvents: Event[] = [
  { id: 1, title: 'Meeting With Jundinu Tech', date: new Date(2024, 0, 4), startTime: '08:00', endTime: '09:30', attendees: ['JD', 'AT', 'BK'] },
  { id: 2, title: 'Meeting With Nonuda Company', date: new Date(2024, 0, 4), startTime: '10:00', endTime: '11:00', attendees: ['NC', 'MR', 'EL'] },
  { id: 3, title: 'Meeting With Nonuda Company', date: new Date(2024, 0, 4), startTime: '14:00', endTime: '15:00', attendees: ['NC', 'MR', 'EL'] },
]
...
function CalendarHeader({ currentDate, onPrevMonth, onNextMonth, onYearChange }: CalendarHeaderProps) {
  return (
    <div className="flex items-center justify-between mb-4">
      ...
      <div className="flex items-center space-x-2">
        <h2 className="text-lg font-semibold">{months[currentDate.getMonth()]}</h2>
        <Select onValueChange={onYearChange} defaultValue={currentDate.getFullYear().toString()}>
...
function CalendarGrid({ currentDate, onSelectDate, events }: CalendarGridProps) {
          ...
          <CardContent className="p-4">
            ...
            <div className="flex mt-2">
              {event.attendees.map((attendee, index) => (
                <Avatar key={index} className="h-6 w-6 -ml-2 first:ml-0 border-2 border-white">
                  <AvatarFallback className="bg-gray-200 text-gray-600 text-xs">
                    {attendee}
                  </AvatarFallback>
                </Avatar>
              ))}
              <span className="text-sm text-gray-500 ml-2">{event.attendees.length} people</span>
            </div>
          </CardContent>
        </Card>
      ))}
  ...
  return (
    <div className="flex-1 ml-6">
      {timeSlots.map(hour => {
        ...
        const eventAtThisHour = events.find(e => 
        ...
        return (
          <div key={hour} className="flex mb-4">
            <div className="w-16 text-right text-sm text-gray-400 pr-4">{timeString}</div>
            <div className="flex-1 min-h-[40px]">
              {eventAtThisHour && (
                <Card className={`p-2 ${eventAtThisHour.title.includes('Jundinu') ? 'bg-red-50' : 'bg-blue-50'}`}>
                  <CardContent className="p-2">
                    <h4 className="font-semibold">{eventAtThisHour.title}</h4>
                    <p className="text-sm text-gray-500">{eventAtThisHour.date.toLocaleDateString()}</p>
                    <div className="flex mt-2">
...
function AddEventDialog({ isOpen, onClose, onAddEvent }: AddEventDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const attendeesArray = typeof newEvent.attendees === 'string' 
...
export function InteractiveCalendarUi() {
    ...
    <div className="flex p-6 bg-white rounded-lg shadow-lg">
      <div className="w-64">
        ...
        <CalendarGrid
          ...
          events={events}
        />
        <EventList events={events.filter(e => e.date.toDateString() === currentDate.toDateString())} />
        <Button onClick={() => setIsAddEventDialogOpen(true)} className="w-full mt-4">
